...
\item \textit{Macbeth}: “A real man acts boldly. Macbeth hesitates to kill Duncan. Therefore Macbeth is not a real man.”
\item \textit{Julius Caesar}: “Brutus is honorable. Honorable men always act justly. Therefore Brutus's killing of Caesar must be just.”
\end{enumerate}

\section*{Part 2: Repair the Argument}
Choose one argument from Part 1, preferably one of the Shakespeare examples if you know the text. Rewrite it so that the key term is used in only one clear sense. Then explain whether the repaired argument now supports its conclusion.

\vspace{4cm}

\section*{Part 3: Create}
Write two short arguments on the same topic. You may use a public issue, a school issue, \textit{Macbeth}, or \textit{Julius Caesar}.
...
\section*{Exit Claim}
\textbf{Portfolio exit:} In three or four sentences, explain how the habit of fixing terms connects to the four-step analysis you learned in Week 1. Name one word from \textit{Macbeth} or \textit{Julius Caesar} that would need careful definition before an argument about the play could be judged.

...
In Week 2 we learned that words can shift their meaning and break arguments. In this week we move one step further. Even when the meaning of our terms is fixed, we can still make unclear or misleading claims if we do not pay attention to how we combine terms into statements.
...
This reading introduces the basic structure of propositions and the most common ways claims can be stated clearly or unclearly. Because this unit now functions as a twelfth-grade long logic course, we will connect proposition work to the recurring literary anchors \textit{Macbeth} and \textit{Julius Caesar}. Claims such as “Macbeth is fated,” “Caesar is ambitious,” or “Brutus is honorable” cannot be judged well until we know exactly what is being asserted.
...
- In an O proposition (“Some S are not P”), the subject term S is not distributed, because the claim concerns only some members of S. The predicate term P, however, is distributed. Consider “Some hikers are not climbers.” We are discussing only part of the hiker class, but the selected hiker or hikers are excluded from the climber class as a whole. The proposition does not merely say that they are not some climbers---a claim that would be true even of people who are climbers. It says that they are not any member of P. Because the denial reaches the whole predicate class, P is distributed.
...
\item In an O proposition (“Some S are not P”), which term is distributed and which is undistributed?
\item Why can the statement “Politicians are dishonest” lead to misunderstanding in an argument?
\item Translate one claim from \textit{Macbeth} or \textit{Julius Caesar} into clearer proposition form. Possible claims: “Macbeth is fated,” “Caesar is ambitious,” “Brutus is honorable,” or “Antony is loyal.”
\end{enumerate}

\section*{Window Note and Summary}
Create a portfolio window note that shows one proposition written in four different ways by changing only its quality and quantity (A, E, I, and O forms). You may use a neutral example or a claim from \textit{Macbeth} or \textit{Julius Caesar}. For each version, include a small drawing or symbol that represents what the claim is asserting. At the bottom of the window note, write a 5--7 sentence summary explaining why being precise about quality and quantity helps us evaluate whether one claim follows from another.

\section*{Connection to Week 2}
How does the ability to identify the exact quantity and quality of a claim help us avoid the kind of ambiguity we studied in Week 2 (when a word shifts meaning)?
